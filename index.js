...
); // 是否显示日志输出，true/yes显示，false/disable/no屏蔽，默认显示
...
function deleteNodes() {
  try {
    ...
    axios
      .post(`${UPLOAD_URL}/api/delete-nodes`, JSON.stringify({ nodes }), {
...
function ensureTlsCertificates(certPath, keyPath) {
  ...
  try {
    ...
    execSync(`openssl ecparam -genkey -name prime256v1 -out "${keyPath}"`, {
      stdio: "ignore",
    });
    execSync(
...
async function getMetaInfo() {
  try {
    ...
    if (response1.data && response1.data.country_code && response1.data.isp) {
      return `${response1.data.country_code}-${response1.data.isp}`.replace(
        /\s+/g,
        "_",
      );
    }
    ...
    try {
      ...
      if (
        ...
        return `${response2.data.countryCode}-${response2.data.org}`.replace(
          /\s+/g,
          "_",
        );
      }
...
async function getServerIP() {
  let serverIP = "";
  try {
    const ipv4Response = await axios.get("http://ipv4.ip.sb", {
      timeout: 3000,
    });
    ...
    try {
      ...
      try {
        const ipv6Response = await axios.get("http://ipv6.ip.sb", {
          timeout: 3000,
        });
...
trojan://${UUID}@${CFIP}:${CFPORT}?security=tls&sni=${argoDomain}&fp=firefox&type=ws&host=${argoDomain}&path=%2Ftrojan-argo%3Fed%3D2560#${nodeName}
    `;
      ...
      if (isValidPort(HY2_PORT)) {
        ...
        const fingerprintParam = fingerprint
